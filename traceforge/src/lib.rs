...
impl ConfigBuilder {
    ...
    /// Enables temporal verification with the given transit bounds
    /// and storage delay.
    ///
    /// * `l`: minimum network transit time
    /// * `u`: maximum network transit time
...
///
/// `sleep(d)` is a first-class event in the execution graph. When
/// [`ConfigBuilder::with_temporal`] is set, its sole effect on the
/// temporal semantics is to shift the thread's `[τ_lo, τ_hi]` window
/// by `duration`.
///
/// In legacy runs (no `with_temporal`), `duration` is ignored by the
/// model checker: the event is still recorded and appears in printed
/// traces, but no temporal feasibility filtering is performed.
