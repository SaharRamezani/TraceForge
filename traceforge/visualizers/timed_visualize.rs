...
//! and per-rejection prune logs for several timed scenarios.
//!
...
//!
//! Run end-to-end:
//!   cargo run --example timed_visualize
...
fn make_config(label: &str, meta: &Meta) -> (PathBuf, Config) {
    ...
    let cfg = Config::builder()
        .with_timed(meta.l, meta.u, meta.sd)
...
// `four_thread_pipeline_timed_pruning` in timed.rs).
// =====================================================================
...
// =====================================================================
// Mirrors `timed::sleep_advances_lower_bound_both_outcomes`.
//
// L=0, U=1000, sd=0; consumer waits 100 ticks; main sleeps 10 then
// sends. Both rf=send and rf=⊥ branches are timed consistent, so
...
fn run_sleep_advances(label: &str) {
    let meta = Meta {
        ...
        description:
            "Wide transit window: rf=send and rf=⊥ are both timed consistent. \
...
// =====================================================================
// Mirrors `timed::finite_wait_forces_timeout`.
//
...
// =====================================================================
// Mirrors `timed::infinite_wait_prunes_timeout`.
//
...
// =====================================================================
// Mirrors `timed::predicate_timed_recv`.
//
...
// the s1 send is timed consistent and admits both rf=s1 and ⊥.
// Expect 2 execs.
// =====================================================================
    ...
    let meta = Meta {
        ...
        description:
            "Two senders, but the recv predicate only matches s1. s2's send is filtered \
             out by the predicate (not by the timed filter). Expect 2 execs.",
...
fn main() {
    ...
    // First four `timed::*` tests, captured for the viewer.
