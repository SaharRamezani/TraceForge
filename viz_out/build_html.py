...
execution graphs.
To add a new protocol (3PC, Raft, Paxos, ...): write a Rust visualize test that
...
meta.json fields (all optional):
  ...
  nondet_role  "vote" | "crash" | "role" | "auto"
                  How to render NONDET nodes in the graph.
                  vote   = true → "vote Yes", false → "vote No" (2PC etc.)
                  crash  = true → "crash",    false → "continue" (Skeen 3PC)
                  role   = true → "Leader",   false → "Follower" (leader-
                           election protocols where coord() picks a branch)
                  auto (default) = heuristic: >1 NONDET per thread ⇒ crash,
                           else vote.
...
# Don't anchor on the trailing `>` timed mode appends `<br/>τ ∈ [..]`.
...
def prettify_dot(dot: str, labels: dict, nondet_role: str = "auto") -> str:
    ...
    #              recv boundary)
    #   "role"  -> NONDET true → "Leader",   false → "Follower"
    #             (per-ballot role selection in leader-election
    #              protocols where coord() picks a branch, not a vote)
    # When `nondet_role` is missing or "auto", fall back to a whole-
    # execution heuristic: any thread with >1 nondet ⇒ crash semantics,
    # otherwise vote.

    if nondet_role in ("vote", "crash", "role"):
        force = nondet_role
    else:
    ...
    def repl_nondet(m):
        ...
        if force == "crash":
            ...
            inner = ('<font color="#1f8b3a"><b>Leader</b></font>' if val == "true"
                     else '<font color="#3366aa"><b>Follower</b></font>')
        else:
...
<head>
<meta charset="utf-8">
<title>TraceForge: timed pipeline executions</title>
<style>
  :root {
    --bg: #0e1116;
    --panel: #161b22;
  ...
  * { box-sizing: border-box; }
  ...
  /* "prune-caused block": block whose stuck recv had at least one
     timed prune fire during the same exec. Uses the same orange
...
function updateLegend() {
  ...
  if (mode === "prunes") {
    legendEl.innerHTML = `
      ...
      <span class="pill">reason: <code>timed</code> (empty τ) or <code>symmetric</code> (POR dup)</span>
  ...
  if (mode === "execs") {
    run.execs.forEach((ex, i) => {
      ...
      if (ex.blocked) {
        ...
        // Differentiate "block whose recv was emptied by the timed
        ...
        if (pCount > 0) {
          pill = `<span class="prune-block-pill" title="${pCount} timed prune(s) fired during this exec">prune-caused block</span>`;
        } else {
  ...
  if (mode === "execs") {
    ...
    if (ex.blocked) {
      ...
      const extra = pCount > 0
        ? ` &nbsp;·&nbsp; <b>${pCount}</b> timed prune(s) fired during this exec`
        : "";
